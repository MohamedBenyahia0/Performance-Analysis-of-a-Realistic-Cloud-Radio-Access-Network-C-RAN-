...
bits1 = randi([0,1],N,1); % generation des messages d'information pour  BCH1
...
codeWords1=reshape(codeWords1,[N+(31-k1)*nb_word,1]);
symboles1=bits2symbols(codeWords1,'PSK',2);
...
if strcmp(equalizer,'ZF')
    s_est1 = ZFequalizer(Z1,H, 'PSK',2) ; % prise de décision

else %DF
    s_est1 = DFequalizer(Z1,H, 'PSK',2) ; % prise de décision
end
...
bits_hat1=reshape(m_hat1,[N,1]);

nb_err1(ii)=nb_err1(ii)+sum(abs(bits_hat1-bits1),"all");
BER1(ii) = nb_err1(ii)/(N);  % calcul du taux d'erreur binaire
end
end
