...
bits2 = randi([0,1],N_bits,1); % generation des messages d'information pour  BCH2
...
codeWords2=reshape(codeWords2,[N_bits+(31-k2)*nb_word,1]);

%% %%%%%%%%% BPSK modulation

symboles2=bits2symbols(codeWords2,mod,M);
symboles2=reshape(symboles2,[nb_word,31]);

%% Application du canal AWGN

W2 =sqrt(No(ii)/2)* (randn(nb_word,31) + i*randn(nb_word,31));  % bruit
Z2=H*symboles2+W2;
...
bits_hat2=reshape(m_hat2,[N_bits,1]);

%%   %%%%%%%   calcul du BER

nb_err2(ii)=nb_err2(ii)+sum(abs(bits_hat2-bits2),"all");

BER2(ii) = nb_err2(ii)/(N_bits);  % calcul du taux d'erreur binaire
